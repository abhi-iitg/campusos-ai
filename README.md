...
CampusOS AI is an AI PM case study exploring how I would define, design, evaluate, and launch an AI-native campus operating system. It demonstrates product strategy, AI system design, evaluation frameworks, and go-to-market thinking — grounded in a working prototype with a real eval harness.

<p align="center">
  <a href="https://github.com/abhi-iitg/campusos-ai.git"><img src="https://img.shields.io/badge/💻_GITHUB-Repository-181717?style=for-the-badge&logo=github" alt="GitHub"/></a>
...
🔗 **[Live Demo →](https://abhi-iitg.github.io/campusos-ai/)**
...
![CampusOS AI — Product, Architecture, and Evaluation Overview](docs/assets/campusos-ai-overview.png)

*Visual overview of the product concept, system architecture, evaluation framework, roadmap, and go-to-market strategy.*
...
- [About](#about)
- [Scope and Prototype Disclosure](#scope-and-prototype-disclosure)
- [License](#license)
...
CampusOS AI is that reframed product: designed and documented the way I'd approach it as an AI Product Manager.
...
CampusOS AI unifies the fragmented campus experience into a single AI-native platform. The core insight: **universities aren't unintelligent — their intelligence is inaccessible.** Every policy, deadline, and procedure is documented somewhere. CampusOS AI makes that knowledge retrievable through an AI copilot that understands university-specific context and responds with citations, not hallucinations.
...
```mermaid
flowchart LR
    U[Students / Faculty / Admin] --> W[React + Vite Web App]
    W --> A[Express API]
    A --> K[Institution Knowledge Base]
    K --> D{Confidence / Policy Check}
    D -->|Grounded answer| C[Citation-Based Copilot]
    D -->|Low confidence| E[Human Escalation]
    D -->|Sensitive query| R[Crisis / Support Resources]
    A --> M[Mock Campus Modules & Metrics]
    A --> H[Offline Evaluation Harness]
    H --> Q[Quality Metrics & Product Decisions]
```
...
Student / Faculty / Admin
        │
        ▼
CampusOS AI Web App (React + Vite)
        │
        ▼
...
The repository includes product visuals for the AI copilot, admin intelligence dashboard, and student command center.

| Product Surface | Purpose |
|---|---|
| AI Campus Copilot | Citation-based answers and safe escalation |
| Admin Intelligence Dashboard | Query trends, escalation rates, and knowledge gaps |
| Student Command Center | Unified deadlines, tasks, and campus alerts |

---

## Project Highlights

- **Product strategy:** Problem framing, personas, customer wedge, phased roadmap, and GTM motion.
- **AI safety:** Citation-first responses, confidence thresholds, escalation, and crisis-resource handling.
- **Evaluation:** Offline harness against a labeled dataset using the same copilot logic as the UI.
- **Business orientation:** Product decisions connected to groundedness, ticket deflection, satisfaction, and adoption.
- **Execution evidence:** Working prototype, architecture documentation, PRD, evaluation plan, experiment backlog, and launch plan.
...
│   ├── assets/
│   │   ├── screenshots/              # Admin intelligence, command center, copilot demo
│   │   └── campusos-ai-overview.png  # Project overview visual
│   └── product/
│       ├── PRD.md
...
```bash
git clone https://github.com/abhi-iitg/campusos-ai.git
cd campusos-ai
...
The overview infographic is included at `docs/assets/campusos-ai-overview.png`. Additional product screenshots remain in `docs/assets/screenshots/`.

---

## Scope and Prototype Disclosure

The current repository is a product and engineering prototype. The copilot uses a scripted knowledge base as a controlled stand-in for a production RAG pipeline, while the future-state architecture describes how institutional retrieval, identity, analytics, and campus-service integrations could be implemented. This distinction is intentional: it allows the evaluation harness to validate decision logic before introducing live-model cost, latency, and operational risk.
